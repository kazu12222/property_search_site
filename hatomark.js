...
(async () => {
  ...
  const search_button = await getElements('button[type="submit"', /^検索$/);
  search_button.click();
  await page.waitForNavigation();
  const list_table = await page.$(".list-table");
  const building_li = await list_table.$$(".col-12"); //偶数でカード
  ...
  async function getLink() {
    const detail_link_base = await getElements(
      "a",
      /詳細を見る/,
      building_li[i]
    );
  ...
  async function getElements(Elementname, textContent, A = page) {
